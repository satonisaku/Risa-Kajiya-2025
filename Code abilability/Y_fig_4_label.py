...
def plot_nrem_fr(rat_name):
    ...
    #vCA1_N, vCA1_R =joblib.load(data_dir + rat_name + '_vCA1ex_active_2.joblib')
    ...
    for n in range(vCA1_N.shape[0]):
        for m in range(vCA1_N.shape[0]):
            if n<m:
                nn=vCA1_N[n]*vCA1_N[m]
                NN=np.append(NN,nn)

    RR=[]
    for n in range(vCA1_R.shape[0]):
        for m in range(vCA1_R.shape[0]):
            if n<m:
                rr=vCA1_R[n]*vCA1_R[m]
                RR=np.append(RR,rr)

    NR=[]
    for n in range(vCA1_N.shape[0]):
        for m in range(vCA1_R.shape[0]):
                nr=vCA1_N[n]*vCA1_R[m]
                NR=np.append(NR,nr)

    com=NR.reshape(-1,vCA1_N.shape[0])
    NR2= com[np.triu_indices(com.shape[0], k=1)]

    RN=[]
    for n in range(vCA1_R.shape[0]):
        for m in range(vCA1_N.shape[0]):
                rn=vCA1_R[n]*vCA1_N[m]
                RN=np.append(RN,rn)
    com=RN.reshape(-1,vCA1_R.shape[0])
    RN2= com[np.triu_indices(com.shape[0], k=1)]

    fig = plt.figure(figsize=(6, 6))

    data_dir=r'C:\Users\saton\OneDrive\デスクトップ\\data3\\'
    joblib.dump((NN,RR,NR2,RN2), os.path.expanduser(data_dir+rat_name+'_Y_vCA1ex_NN_RR_NR_RN.joblib'), compress=3)
