...
def plot_nrem_fr(rat_name):
    ...
    #PL5_N = np.where(vCA1_N == 1)[0]
    #PL5_R = np.where(vCA1_R == 2)[0]
    BLA_N, BLA_R = joblib.load(data_dir + rat_name + '_Y_BLAex_active_2_50_2.joblib')
    PL5_N = np.where(BLA_N == 1)[0]
    PL5_R = np.where(BLA_R == 2)[0]
    ...
    vCA1_N = np.array(PL5_N, dtype=int)
    vCA1_R = np.array(PL5_R, dtype=int)
    tg_N = tg[vCA1_N]
    tg_R = tg[vCA1_R]
    ...
    for e, event in enumerate(events):
        ...
        for n in range(len(TT2[e])):
            ...
            fr = spikecount_in / (time_edge[::2][1:] - time_edge[1::2][:-1])
            spike_in.append(fr)
    ...
    data_dir=r'C:\Users\saton\OneDrive\デスクトップ\\data3\\'
    #joblib.dump((correlation_matrix1,Corr_mean), os.path.expanduser(data_dir+rat_name+'_PL5ex_BLAex_R_R_SWR_corr.joblib'), compress=3)
    joblib.dump((fr1_1_swr, fr2_1_swr, fr3_1_swr), os.path.expanduser(data_dir + rat_name + '_Y_BLAex_unit_fr_swr_2.joblib'),compress=3)
    joblib.dump((fr1_1_spindle,fr2_1_spindle,fr3_1_spindle), os.path.expanduser(data_dir + rat_name + '_Y_BLAex_unit_fr_spindle_2.joblib'),compress=3)
    joblib.dump((fr1_1_crip,fr2_1_crip,fr3_1_crip), os.path.expanduser(data_dir + rat_name + '_Y_BLAex_unit_fr_crip_2.joblib'),compress=3)
    joblib.dump((fr1_1_hfo,fr2_1_hfo,fr3_1_hfo), os.path.expanduser(data_dir + rat_name + '_Y_BLAex_unit_fr_hfo_2.joblib'),compress=3)
    #joblib.dump((fr1_1, fr1_2, fr1_3), os.path.expanduser(data_dir + rat_name + '_vCA1ex_unit_fr_swr.joblib'),compress=3)
