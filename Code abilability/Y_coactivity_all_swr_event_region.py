...
def plot_nrem_fr(rat_name):
    ...
    for e, event in enumerate(events):
        ...
        for x in range(na.shape[0]) :
            for y in range(nb.shape[0]):
                ...
                z = (nab - E) / sd
                RR=np.append(RR,z)
        com = RR.reshape(-1, na.shape[0])
        RR = com[np.triu_indices(com.shape[0], k=1)]
        RR2.append(RR)
    ...
    data_dir=r'C:\Users\saton\OneDrive\デスクトップ\\data3\\'
    joblib.dump((RR2[0]), os.path.expanduser(data_dir+rat_name+'_Y_vCA1ex_coactivity_all_nrem_swr.joblib'), compress=3)
    joblib.dump((RR2[1]), os.path.expanduser(data_dir + rat_name + '_Y_vCA1ex_coactivity_all_nrem_spindle.joblib'), compress=3)
    joblib.dump((RR2[2]), os.path.expanduser(data_dir + rat_name + '_Y_vCA1ex_coactivity_all_nrem_crip.joblib'),compress=3)
    joblib.dump((RR2[3]), os.path.expanduser(data_dir + rat_name + '_Y_vCA1ex_coactivity_all_nrem_hfo.joblib'), compress=3)
